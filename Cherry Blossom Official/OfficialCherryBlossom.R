...
prediction <- rowMeans(predict(model, newdata = data2024DCKYLI))

lower <- apply(predict(model, newdata = data2024DCKYLI), 1, quantile, prob = .025)
upper <- apply(predict(model, newdata = data2024DCKYLI), 1, quantile, prob = .975)
...
newyorkpre <- mean(rowMeans(predict(model, newdata = newyork)))
nylower <- quantile(colMeans(predict(model, newdata = newyork)), prob = .025)
nyupper <- quantile(colMeans(predict(model, newdata = newyork)), prob = .975)
